...
public class NoteServiceImpl implements NoteService {
	...
	public NoteResult updateNote(String noteId, String title, String body) {
		 ...
		 note.setCn_note_body(body);//设置内存
		 Long time = System.currentTimeMillis();
		 note.setCn_note_last_modify_time(time);//设置最后一次修改时间
//		 int rows = noteDao.updateNote(note);//更新表
	...
	public NoteResult addNote(String userId, String bookId, String noteTitle) {
		...
		note.setCn_note_id(bookId);//设置笔记本ID
...
//		noteDao.updateTypeId(noteId);
		...
		share.setCn_share_body(note.getCn_note_body());//分享的笔记内容
		shareDao.save(share);
		//创建返回结果
		result.setStatus(0);
